import { type GameMode } from '@/mechanics/services'
import { type EnemiesKilledState } from '@/mechanics/services/Statistics/types'
...
export class ScoreScreen extends Screen<ScoreScreenState> {
  ...
  renderHeader() {
    ...
    this.overlay.renderElement({
      ...
      text: !state.username ? 'ЗАЛОГИНЬСЯ, ЧТОБЫ ПОПАСТЬ В ТОП' : ' ',
      align: 'center',
    })
  ...
  renderCategory({ count, enemyTankSprite, score }: CategoryData) {
    this.currentPosY += 5
